...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // Global memory directory — writable for main so the agent can edit global SYSTEM.md
    // and global memory files. Non-main groups only get read-only access.
  ...
  const groupAgentDir = path.join(DATA_DIR, 'sessions', group.folder, '.agent');
  fs.mkdirSync(groupAgentDir, { recursive: true });
  mounts.push({
    hostPath: groupAgentDir,
